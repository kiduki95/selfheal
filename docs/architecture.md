...
>
> 이 문서는 **우산(umbrella) 문서**다. 각 레이어의 상세 스펙은 별도 문서가 가진다([processing-layer.md](./processing-layer.md), [codeflow-layer.md](./codeflow-layer.md)). 여기서는 (1) 레이어가 어떻게 맞물리는지, (2) 공유 데이터 모델, (3) **UI ↔ 백엔드 컨트랙트**, (4) 디렉터리 구조와 빌드/배선 순서를 정의한다.
>
...
UI는 `web/`의 **Vite + React + TypeScript** 목업이다(`web/src/`, `npm --prefix web run build` → `web/dist`를 Hono가 서빙). 8 페이지 + 온보딩/오버레이가 **전부 `src/data/`의 mock 모듈**로 동작한다. 백엔드 배선 = 페이지별로 mock import를 `/api/*` JSON으로 교체하는 작업.

### 3.1 페이지 → 데이터 → 백엔드 매핑

| 페이지 (`web/src/pages/*.tsx`) | mock 모듈 | `/api` 엔드포인트 | 백엔드 원천 | 배선 가능? |
|---|---|---|---|---|
...
- `src/api/static.ts` — `web/dist/`(Vite 빌드 산출물) 정적 서빙. catch-all. (개발 중엔 `npm --prefix web run dev`의 Vite 서버가 HMR + `/api` 프록시 제공.)
- `scripts/serve.ts` — `@hono/node-server`로 위 앱을 띄우는 **얇은 부트스트랩**뿐.

UI 쪽은 Vite로 빌드(`web/`는 자체 `package.json`을 가진 독립 프론트엔드 패키지 — 브라우저/DOM 타입을 백엔드 node 타입과 분리). 배치 레이어(Ingestion/Processing/…)는 **프레임워크 없이** 모듈+CLI 래퍼 유지 — 오케스트레이션(큐/스케줄)은 무인 루프가 필요해질 때 pg-boss(기존 Postgres 재사용)로 핀포인트 도입.
...
│     ├─ static.ts        web/dist/ 정적 서빙 (catch-all)
│     └─ routes/          리소스별 라우터 (graph, proposals, reviews, dashboard, planned)
├─ web/                   Vite + React + TypeScript 목업 UI (자체 package.json)
│  ├─ package.json        프론트엔드 deps + dev/build 스크립트
│  ├─ vite.config.ts      base './', /api → :5175 프록시, → dist 빌드
│  ├─ tsconfig.json       DOM lib · jsx react-jsx · strict
│  ├─ index.html          Vite 엔트리 (src/main.tsx 로드)
│  ├─ dist/               빌드 산출물 (Hono가 서빙, gitignore)
│  └─ src/
│     ├─ main.tsx         createRoot 엔트리
│     ├─ app.tsx          셸(사이드바/탑바/라우터)
│     ├─ types.ts         공통 타입(Route 등)
│     ├─ pages/*.tsx      페이지 (dashboard·sources·reviews·processing·insights·agent·activity·onboarding·settings)
│     ├─ data/            mock.ts · mock-extras.ts  목업 데이터 + export 타입 (배선 시 /api로 교체 대상)
│     ├─ components/      ui.tsx · icons.tsx · overlays.tsx
│     └─ styles.css
├─ scripts/               실행 스크립트 (migrate, seed, run-corpus, insight, codeflow-scan)
