...
UI 쪽은 빌드 스텝 없음(의도적, codeflow 비용철학과 같은 결: 도구 최소화). 배치 레이어(Ingestion/Processing/…)는 **프레임워크 없이** 모듈+CLI 래퍼 유지 — 오케스트레이션(큐/스케줄)은 무인 루프가 필요해질 때 pg-boss(기존 Postgres 재사용)로 핀포인트 도입.
